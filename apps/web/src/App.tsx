    ...
    if (this.state.hasError) {
      return (
        <div style={{
          ...
          <p>This error occurred in the React app:</p>
          <pre style={{
...
function App() {
  ...
  console.log('📱 Running in Capacitor?', !!(window as any).Capacitor);
  console.log('🌐 Current location:', window.location.href);
  console.log('🔑 Initial token from localStorage:', token ? 'present' : 'null');
  ...
  const checkConnectivity = async () => {
    try {
      ...
      const response = await fetch('https://staging.slushdating.com/api/health', {
        method: 'GET',
  ...
  useEffect(() => {
    console.log('🔄 useEffect triggered for user/token loading');
    try {
      const savedUser = localStorage.getItem('user');
      console.log('💾 Saved user from localStorage:', savedUser ? 'present' : 'null');
      if (savedUser && token) {
        const parsedUser = JSON.parse(savedUser);
        console.log('👤 Setting user from localStorage:', parsedUser?.name || parsedUser?.email);
        setUser(parsedUser);
      } else {
        console.log('❌ No saved user or token, staying logged out');
      }
    } catch (error) {
  ...
  console.log('🎯 Render decision time:');
      ...
      <BrowserRouter basename={import.meta.env.VITE_USE_SUBPATH === 'true' ? '/app' : '/'}>
        <Routes>
          ...
          <Route
            ...
            element={
              !token ? (
                console.log('🔐 No token - showing AuthFlow'),
                ...
                console.log('💕 User authenticated and onboarded - showing DatingApp'),
...
function DatingApp({ user, onLogout }: { user: any; onLogout: () => void }) {
  ...
  const fetchUserBookings = useCallback(async () => {
    try {
      ...
      console.log('[Bookings] Received bookings:', bookings);
      
      // Set the most recent active booking
      if (bookings && bookings.length > 0) {
        const mostRecentBooking = bookings[0]; // Already sorted by bookedAt desc
        console.log('[Bookings] Most recent booking:', mostRecentBooking);
        
        // Handle both populated and non-populated eventId
        const eventId = mostRecentBooking.eventId?._id || mostRecentBooking.eventId;
        if (eventId) {
          console.log('[Bookings] Setting bookedEventId to:', eventId);
          setBookedEventId(eventId.toString());
        } else {
          console.warn('[Bookings] No eventId found in booking:', mostRecentBooking);
          setBookedEventId(null);
        }
      } else {
        // Clear booked event if no bookings found
        console.log('[Bookings] No bookings found, clearing bookedEventId');
        setBookedEventId(null);
      }
    } catch (error) {
      console.error('[Bookings] Failed to fetch user bookings:', error);
      // On error, don't clear existing bookedEventId to avoid flickering
    }
  }, []);
  ...
  // Refresh bookings when events tab becomes active
  useEffect(() => {
    if (activeTab === 'events' && user) {
      console.log('[Bookings] Events tab activated, refreshing bookings...');
      fetchUserBookings();
      
      // Also set up periodic refresh while on events tab (every 30 seconds)
      const interval = setInterval(() => {
        console.log('[Bookings] Periodic refresh while on events tab...');
        fetchUserBookings();
      }, 30000); // Refresh every 30 seconds
      
      return () => clearInterval(interval);
    }
  }, [activeTab, user, fetchUserBookings]);

  // Refresh bookings when window/tab becomes visible (user returns from native app)
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && user) {
        console.log('[Bookings] Window became visible, refreshing bookings...');
        fetchUserBookings();
      }
    };
