...
export const EventDetail: React.FC<EventDetailProps> = ({ eventId, onBack, onBook, password }) => {
    ...
    useEffect(() => {
        ...
        const fetchBookingStatus = async () => {
            ...
            try {
                console.log('[EventDetail] Fetching booking status for event:', eventId);
                const status = await eventService.getBookingStatus(eventId);
                console.log('[EventDetail] Booking status:', status);
                setIsBooked(status.isBooked);
            } catch (err: any) {
                console.error('[EventDetail] Failed to fetch booking status:', err);
